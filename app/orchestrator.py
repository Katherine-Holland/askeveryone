...
def _last_user_topic(conversation: List[Dict[str, str]]) -> str:
    # crude but effective: last user message trimmed
...
def _extract_destination_like_phrase(text: str) -> str:
    """
    Simple heuristic for follow-ups like:
...
def _derive_conversation_state(
    *,
    ...
    """
    Deterministic “Conversation State” to reduce drift.
    This is NOT generated by an LLM, so it won’t hallucinate new facts.
    """
    conversation = conversation or []
    q = (query or "").strip()
    followup = _detect_followup(q)
...
def _build_messages(
    *,
    ...
    """
    Normalized messages for all providers.
    Providers should prefer meta['messages'] for multi-turn.
    """
